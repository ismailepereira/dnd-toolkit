...
function atacar(atacante, acao, vezes = 1) {
  ...
  // Aviso de reação (assistente): se o ALVO é um PJ com reação defensiva
  // disponível, lembra o Mestre de oferecer a reação antes de aplicar o dano.
  if (alvo.tipo === 'pc' && alvo.fichaId && typeof reacoesDoPC === 'function') {
    const reac = reacoesDoPC(fichas.find(x => x.id === alvo.fichaId));
    if (reac.length) logCombate(`⚡ ${alvo.nome} pode REAGIR: ${reac.join(' · ')} — ofereça antes de aplicar o dano.`);
  }
