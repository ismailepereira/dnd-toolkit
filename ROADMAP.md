...
22. 🧭 **Fase 20 — Navegação & Mobile** (proposta 13/07/2026, a partir de pesquisa UX — sidebar não funciona <768px; bottom-nav de 3–5 itens na zona do polegar é o padrão; alvos ≥44px):
    ...
    - 🔶 **20.4** (parcial, 14/07/2026) — **Passe de responsividade**: auditoria real a 375px (script de varredura de overflow) em todas as abas do Mestre e do Jogador; achado e corrigido bug real de overflow horizontal (Combate/Encontros estouravam a tela) — 7 grids `→ 1 coluna` trocaram `1fr` por `minmax(0, 1fr)` e `.add-monstro` ganhou wrap no mobile. **Falta**: auditar Modo de Jogo com ficha completa (PV/slots/inventário) e o editor de aventuras em canvas — ver `CHANGELOG.md` para o que ficou pendente.
