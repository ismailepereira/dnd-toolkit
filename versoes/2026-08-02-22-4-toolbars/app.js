...
// ===== Fase 22.4: dropdown "⚙️ mais" — fecha ao agir ou ao clicar fora =====
document.addEventListener('click', (e) => {
  const dentro = e.target.closest('.mais-pop');
  // clicou numa AÇÃO dentro do menu (botão) → executa e fecha o menu
  if (dentro && e.target.closest('button')) {
    const det = dentro.closest('.mais-menu');
    if (det) det.open = false;
    return;
  }
  // clicou fora de qualquer menu aberto → fecha todos
  if (!e.target.closest('.mais-menu')) {
    document.querySelectorAll('.mais-menu[open]').forEach(m => { m.open = false; });
  }
});
...
function renderFichas() {
  ...
  // Fase B2: barra "liberar o grupo todo" quando alguém tem próximo nível planejado
  const temPlano = f => !morto2(f) && (f.progressaoPlanejada || []).some(s => s && s.nivel === (f.nivel || 1) + 1);
  function morto2(f) { return f.status === 'morto'; }
  if (fichas.some(temPlano)) {
    const bar = document.createElement('div');
    bar.className = 'fichas-liberar-todos';
    bar.innerHTML = '<button class="btn-secondary" id="btnLiberarTodos">⬆️ Liberar próximo nível de todos</button>';
    listaFichas.appendChild(bar);
    bar.querySelector('#btnLiberarTodos').addEventListener('click', async (e) => {
      const b = e.currentTarget; b.disabled = true;
      try {
        await fetch('/api/fichas/liberar_nivel_todos', { method: 'POST' });
        try { fichas = await (await fetch('/api/fichas')).json(); } catch (err) { }
        renderFichas();
      } catch (err) { alert('Erro ao liberar níveis.'); b.disabled = false; }
    });
  }

  // Fase combate: o descanso é ativado SÓ pelo Mestre (o jogador não recupera
  // slots/recursos sozinho). Barra concede descanso ao grupo todo de uma vez.
  if (fichas.some(f => f.status !== 'morto') && typeof aplicarDescansoCurto5e === 'function') {
    const rb = document.createElement('div');
    rb.className = 'fichas-descanso';
    rb.innerHTML = '<span class="criador-hint-inline">😴 Descanso do grupo:</span> '
      + '<button class="btn-secondary" id="btnDescCurtoTodos">☕ Curto</button> '
      + '<button class="btn-secondary" id="btnDescLongoTodos">🌙 Longo</button>';
    listaFichas.appendChild(rb);
    const aplicarGrupo = (tipo) => {
  ...
  fichas.forEach(f => {
    ...
    const proxPlano = (f.progressaoPlanejada || []).find(s => s && s.nivel === (f.nivel || 1) + 1);
    ...
    card.innerHTML = `
      ...
      ${proxPlano && !morto ? `<div class="ficha-plano-aviso">📈 Plano do jogador pronto até o nível ${(f.progressaoPlanejada[f.progressaoPlanejada.length - 1] || {}).nivel}.</div>` : ''}
      <div class="ficha-card-acoes">
        ...
        ${proxPlano && !morto ? `<button class="btn-primary btn-liberar-nivel" data-liberar="${f.id}" title="Aplica o plano do jogador para o nível ${proxPlano.nivel}">⬆️ Liberar nível ${proxPlano.nivel}</button>` : ''}
      </div>
    `;
    ...
    const btnLib = card.querySelector('[data-liberar]');
    if (btnLib) btnLib.addEventListener('click', async (e) => {
      e.stopPropagation();
      btnLib.disabled = true;
      try {
        const r = await fetch(`/api/fichas/${f.id}/liberar_nivel`, { method: 'POST' });
        const d = await r.json();
        if (d.ok && d.ficha) {
          const i = fichas.findIndex(x => x.id === f.id);
          if (i >= 0) fichas[i] = d.ficha;
          renderFichas();
        } else { alert(d.detalhe || 'Não foi possível liberar o nível.'); btnLib.disabled = false; }
      } catch (err) { alert('Erro ao liberar o nível.'); btnLib.disabled = false; }
    });
...
function parseAcoes(m) {
  ...
  // mantém TODAS as ações (ataques, sopro, magias, habilidades especiais); só o
  // "Multiataque/Múltiplos Ataques" fica de fora, pois tem botão próprio. Ações
  // sem "+X para acertar" viram botões que rolam o dano / descrevem (o Mestre
  // aplica a salva/efeito) — antes elas eram descartadas e sumiam do monstro.
  }).filter(x => !/m[uú]ltiplos? ataques|multiataque/i.test(x.nome));
}
...
function acoesDoPC(f) {
  ...
  // truques: os de dano viram ataque mágico (escala por nível); os SEM dano
  // (Orientação, Consertar, Luz…) viram botões que descrevem o efeito — antes
  // sumiam por não terem dano.
  ...
  if (attr && typeof MAGIAS_DETALHE !== 'undefined') {
    ...
    (f.truques || []).forEach(tq => {
      const d = MAGIAS_DETALHE[tq]; if (!d) return;
      const md = (d.dano && d.dano !== '—') ? String(d.dano).match(/(\d+)d(\d+)\s*([a-zçãéíóôú]+)?/i) : null;
      if (md) {
        ...
        out.push({ texto: `${tq} (truque): ${d.descricao || d.tempo || 'efeito utilitário'}`, nome: tq + ' ✨', bonus: null, dano: null, dmgTipo: null });
      }
    });
  }
  return out;
...
function addMonstro(nome, qtd, tipo = 'monstro') {
  ...
  const acoes = parseAcoes(m);
  // magias do monstro (m.conjuracao): entram como ações "descrever" para o
  // Mestre ter o kit completo da criatura no card (antes só apareciam no bestiário).
  (m.conjuracao || []).forEach(linha => {
    const nome2 = (String(linha).split(':')[0].trim() || 'Magia').slice(0, 40);
    acoes.push({ texto: String(linha), nome: '✨ ' + nome2, bonus: null, dano: null, dmgTipo: null });
  });
  const hp = soNum(m.hp), ca = soNum(m.ca), desMod = mod(m.atributos.des);
  const def = defesasDeTracos(m.tracos), multi = multiataqueDe(m);
  const reacoes = (m.reacoes || []).map(r => String(r)); // aviso de reação (Mestre)
  for (let i = 0; i < qtd; i++) {
    combate.combatentes.push({ id: uid(), tipo, monstroNome: nome, nome: (tipo === 'aliado' ? '🤝 ' : '') + m.nome + (qtd > 1 ? ' ' + (i + 1) : ''), iniciativa: dado(20) + desMod, hpAtual: hp, hpMax: hp, ca, condicoes: [], acoes, multi, reacoes, resist: def.resist, vuln: def.vuln, imune: def.imune });
  }
...
// Teste de Morte pelo tracker (Mestre): rola 1d20 e aplica a regra pura à ficha
// do PJ caído; sincroniza o PV do combatente (recuperou 1 PV ou morreu).
function testeMorteCombate(c) {
  const f = fichas.find(x => x.id === c.fichaId); if (!f) return;
  const res = testeMorte5e(f, dado(20));
  if (res.morto) f.morteEm = new Date().toISOString();
  c.hpAtual = f.hpAtual;
  logCombate(`🎲 Teste de Morte — ${c.nome}: ${res.texto}`);
  salvarFicha(f); salvarCombate(); renderCombate();
}
...
function atacar(atacante, acao, vezes = 1) {
  ...
  // Ação SEM "+X para acertar" (sopro, magia de salva, habilidade): não rola
  // d20 vs CA — rola o dano (se houver) para o Mestre aplicar pelo −Dano / Dano
  // em Área (a salvaguarda é decidida pelo Mestre), ou só descreve o efeito.
  if (acao.bonus == null) {
    const dr = acao.dano ? rolarFormula(acao.dano) : null;
    logCombate(`${atacante.nome} — ${acao.nome}${dr ? `: dano ${dr.txt} = ${dr.total}${acao.dmgTipo ? ' ' + acao.dmgTipo : ''} (aplique no alvo/área)` : `: ${acao.texto}`}`);
  ...
  // Aviso de reação (assistente): se o ALVO tem reação defensiva disponível,
  // lembra o Mestre de oferecer a reação antes de aplicar o dano. PJ = detecta
  // pela ficha; monstro/NPC = usa as reações guardadas do bloco (bestiário).
  let reacAviso = [];
  if (alvo.tipo === 'pc' && alvo.fichaId && typeof reacoesDoPC === 'function') {
    reacAviso = reacoesDoPC(fichas.find(x => x.id === alvo.fichaId));
  } else if (Array.isArray(alvo.reacoes) && alvo.reacoes.length) {
    reacAviso = alvo.reacoes.map(r => String(r).split(':')[0].trim());
  }
  if (reacAviso.length) logCombate(`⚡ ${alvo.nome} pode REAGIR: ${reacAviso.join(' · ')} — ofereça antes de aplicar o dano.`);
...
function renderCombate() {
  ...
  const atualFc = (atual.tipo === 'pc' && atual.fichaId) ? fichas.find(x => x.id === atual.fichaId) : null;
  const atualCaindo = atual.hpAtual === 0 && (!atualFc || atualFc.status !== 'morto');
  turnoInfo.innerHTML = `Rodada <b>${combate.rodada}</b> · Vez de <b>${escapeHtml(atual.nome)}</b>${atualCaindo ? ' <span class="turno-caindo">☠️ caído — role o Teste de Morte no card e encerre o turno</span>' : ''}${alvoSelecionado ? ` · Alvo: <b>${escapeHtml(combate.combatentes.find(c => c.id === alvoSelecionado)?.nome || '—')}</b>` : ''}`;

  listaCombate.innerHTML = '';
  combate.combatentes.forEach((c, i) => {
    const pct = statusPct(c);
    const ehVez = i === idxAtual; // Item 2: destaque do combatente na vez
    ...
    const multiBtn = (c.multi > 1 && (c.acoes || []).some(a => a.bonus != null && a.dano)) ? `<button class="comb-acao multi" data-multi title="Repete o 1º ataque de acerto com dano, ${c.multi}×">⚔ Multiataque ×${c.multi}</button>` : '';
    // defesas
    ...
    // Teste de Morte: PJ caído (0 PV) que ainda não morreu — na vez dele o
    // Mestre rola o teste (o jogador também rola na própria ficha). Mostra
    // sucessos/falhas e destaca quando é a vez dele.
    let morteHtml = '';
    if (c.tipo === 'pc' && c.fichaId && c.hpAtual === 0) {
      const fc = fichas.find(x => x.id === c.fichaId);
      if (fc && fc.status !== 'morto') {
        const s = fc.morteSucessos || 0, fa = fc.morteFalhas || 0;
        morteHtml = `<div class="comb-morte${i === idxAtual ? ' vez' : ''}">
          ☠️ <b>Caindo</b> — ✅ ${s}/3 · ❌ ${fa}/3
          <button class="btn-danger comb-tmorte" data-tmorte title="Rolar Teste de Morte por ${escapeHtml(c.nome)}">🎲 Teste de Morte</button>
          ${i === idxAtual ? '<small> (é a vez dele — role ou passe)</small>' : ''}
        </div>`;
      }
    }
    div.innerHTML = `
      <div class="comb-top">
        ${ehVez ? '<span class="comb-vez-badge" title="É a vez deste combatente">▶ VEZ</span>' : ''}
      ...
      ${acoesHtml || multiBtn ? (ehVez
        ? `<div class="comb-ataques">${multiBtn}${acoesHtml}</div>`
        : `<details class="comb-ataques-fold"><summary>⚔️ Ações</summary><div class="comb-ataques">${multiBtn}${acoesHtml}</div></details>`) : ''}
      ${resumoMPHtml}
    ...
    if (bMulti) bMulti.addEventListener('click', e => { e.stopPropagation(); atacar(c, (c.acoes || []).find(a => a.bonus != null && a.dano), c.multi); });
    const bTMorte = div.querySelector('[data-tmorte]');
    if (bTMorte) bTMorte.addEventListener('click', e => { e.stopPropagation(); testeMorteCombate(c); });
