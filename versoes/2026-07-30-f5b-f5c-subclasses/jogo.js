...
const Jogo = (function () {
  ...
  let jgAlvoId = '';
  let jgFurtoAlvo = ''; // C2: id do alvo com o painel de furto aberto
  ...
  function testeMorte() {
    ...
    const res = testeMorte5e(ficha, r); // regra pura (regras-ficha.js)
    if (res.morto) ficha.morteEm = new Date().toISOString();
    log('🎲 Teste de Morte: ' + res.texto);
    salvar();
    if (res.morto) render();
  }
  ...
  function alternarAcaoTurno(qual, chaveTurno) {
    ...
    a[qual] = !a[qual];
    if (qual === 'acao' && !a.acao) { a.acaoTipo = null; a.ataques = 0; } // desmarcar Ação limpa o tipo/contagem
    ficha.acoesGastas = a;
    salvar();
  }

  // ----- Combate 2b: TRAVA de economia de ação -----
  // A trava só vale EM COMBATE e na MINHA vez (fora disso, sem restrição —
  // treino/preparação). Regra 5e: 1 Ação + 1 Ação Bônus + 1 Reação por turno.
  // Atacar OU conjurar magia de ação gasta a Ação (não os dois); o Ataque Extra
  // permite vários golpes de arma dentro da MESMA Ação (totalAtaques).
  function chaveVezAtual() {
    const t = estadoTurno();
    return (t && t.ativo && t.euNaVez) ? t.chave : null;
  }
  function custoMagiaTempo(nome) {
    const d = (typeof detalheMagia === 'function') ? detalheMagia(nome) : null;
    const tp = (d && d.tempo) || '';
    if (/b[ôo]nus/i.test(tp)) return 'bonus';
    if (/rea[çc][ãa]o/i.test(tp)) return 'reacao';
    return 'acao'; // ação, minutos e rituais tratam-se como Ação no turno
  }
  // Tenta gastar a economia da ação. Fora do meu turno em combate → livre
  // (devolve null, sem gastar). Em combate na minha vez, usa a decisão pura
  // economiaAcao5e (regras-ficha.js): se bloqueia, devolve a mensagem sem gastar;
  // se libera, grava o novo estado (persiste + re-renderiza) e devolve null.
  function tentarGastarEconomia(custo, tipo) {
    if (!custo) return null;
    const t = estadoTurno();
    if (!t || !t.ativo) return null; // fora de combate: sem trava (treino/preparação)
    if (!t.euNaVez) {
      // Turn-based (item 1): fora do meu turno só dá para usar Reação (1×/rodada);
      // Ação e Ação Bônus ficam trancadas até chegar a minha vez.
      if (custo === 'reacao') {
        if (ficha.reacaoUsadaRodada === t.rodada) return 'Você já usou sua Reação nesta rodada.';
        ficha.reacaoUsadaRodada = t.rodada;
        salvar();
        return null;
      }
      return '⏳ Não é a sua vez — fora do seu turno só dá para usar Reação.';
    }
    const max = (typeof totalAtaques === 'function') ? totalAtaques(ficha) : 1;
    const r = economiaAcao5e(acoesDoTurno(t.chave), custo, tipo, max);
    if (r.bloqueio) return r.bloqueio;
    ficha.acoesGastas = Object.assign({}, r.gasto, { chave: t.chave });
    salvar();
    return null;
  }
  function resetEconomiaTurno() {
    // escape hatch: Surto de Ação, Pressa (Haste) ou correção do Mestre
    ficha.acoesGastas = null;
    log('↩️ Economia de ação reiniciada (ação extra / correção).');
    salvar();
  }
  // Envolve o handler de um poder de classe na economia: gasta o custo
  // ('acao'|'bonus') antes de executar; se já foi gasto no turno, bloqueia.
  function comEconomia(custo, fn) {
    return function () {
      const bloq = tentarGastarEconomia(custo, 'poder');
      if (bloq) { log('🚫 ' + bloq); render(); return; }
      return fn.apply(this, arguments);
    };
  }
  // Surto de Ação (Guerreiro): concede uma AÇÃO extra — libera a Ação já gasta
  // deste turno (não é um custo, é um ganho).
  function liberarAcaoExtra() {
    const chave = chaveVezAtual();
    if (!chave) return;
    const g = acoesDoTurno(chave);
    g.acao = false; g.acaoTipo = null; g.ataques = 0;
    ficha.acoesGastas = Object.assign({}, g, { chave });
    salvar();
  }
  ...
  // ----- F5c: Canalizar Divindade ESPECÍFICA do juramento/domínio -----
  // Compartilha o recurso 'Canalizar Divindade' com o Expulsar. Descritiva: o
  // botão gasta 1 uso e loga o efeito; o Mestre resolve salva/dano no combate.
  function classeCanalizar() {
    return classesFicha().find(c => (c.classe === 'Clérigo' && c.nivel >= 2) || (c.classe === 'Paladino' && c.nivel >= 3));
  }
  function maxCanalizar(c) {
    if (!c) return 0;
    return c.classe === 'Clérigo' ? (c.nivel >= 18 ? 3 : c.nivel >= 6 ? 2 : 1) : 1;
  }
  function usarCanalizarSubclasse(nome, desc) {
    const c = classeCanalizar();
    if (!c) return;
    const max = maxCanalizar(c);
    const usados = ficha.recursosUsados['Canalizar Divindade'] || 0;
    if (usados >= max) { log('Sem usos de Canalizar Divindade — recupere num descanso curto.'); render(); return; }
    ficha.recursosUsados['Canalizar Divindade'] = usados + 1;
    log(`✨ Canalizar Divindade — ${nome}: ${desc}`);
    salvar();
  }
  ...
  // Descanso (só o Mestre ativa na fase de combate) — delega à regra pura
  // aplicarDescanso*5e (regras-ficha.js), reusada pelo tracker do Mestre (app.js).
  function descansoCurto() { aplicarDescansoCurto5e(ficha); salvar(); }
  function descansoLongo() { aplicarDescansoLongo5e(ficha); salvar(); }
  ...
  function render() {
    ...
    // atributos
    // 20.5: cada atributo é clicável — toca e rola o teste de atributo puro
    // (d20 + mod). Perícias e salvaguardas já rolavam; faltava o atributo.
    const attrHtml = ATRIBUTOS.map(at => `<button type="button" class="jg-attr jg-attr-roll" data-atributo="${at.chave}" data-anome="${esc(at.nome)}" data-bonus="${m(a[at.chave])}" title="Rolar teste de ${esc(at.nome)}"><span>${getAttrIcone(at.chave)} ${at.nome.slice(0, 3).toUpperCase()}</span><b>${a[at.chave]}</b><i>${fmt(m(a[at.chave]))}</i></button>`).join('');
    ...
    if (_ehConj && (truquesF.length || grimorioF.length || subclasseF.length)) {
      ...
      if (subclasseF.length) {
        ...
        const icone = rotulo === 'juramento' ? '⚜️' : rotulo === 'domínio' ? '🕮' : rotulo === 'patrono' ? '👁️' : '✨';
    ...
    if (_ehConj && (truquesF.length || grimorioF.length || subclasseF.length)) {
        ...
        return `<details class="jg-cast${sem ? ' esgotado' : ''}">
          <summary>
            <div class="jg-cast-info"><b>${esc(nome)}</b> <span class="jg-cast-oquefaz">ⓘ o que faz</span>
              <small>${d.nivel === 0 ? 'truque' : d.nivel + 'º círculo'}${d.nivel > 0 && typeof prepSet !== 'undefined' && prepSet.has(nome) ? ' · 🧠 preparada' : ''} · ${esc(d.escola || '')} · ${tempoIcone(d.tempo)} ${esc(d.tempo)} · ${esc(d.alcance)}${dano ? ` · <span class="jg-dano">${esc(dano)}</span>` : ''}${dt ? ' · ' + esc(dt) : ''}${/concentração/i.test(d.duracao || '') ? ' · 🧠 concentração' : ''}</small>
            </div>
      ...
      // Combate 2a: TODAS as magias de círculo à mão no turno (conhecidas +
      // domínio/juramento + preparadas), não só as preparadas — assim o clérigo
      // não fica só com armas por não ter passado no passo de preparar. O
      // "preparar" segue existindo (bloco 🧠 Magias) como destaque/limite opcional.
      const prepSet = new Set(f.preparadas || []);
      const castaveis = [...new Set([...(f.preparadas || []), ...grimorioF, ...subclasseF])]
      ...
      castHtml = `<div class="jg-bloco jg-conjuracao" data-bloco-acao="magias"><h4>✨ Conjuração ${cabecalho ? `<small>${cabecalho}</small>` : ''}</h4>
        ...
        ${(!truquesF.length && !castaveis.length) ? '<div class="criador-hint">Nenhuma magia conhecida ainda — aprenda/prepare no bloco 🧠 Magias, mais abaixo.</div>' : ''}
      </div>`;
    }
    ...
    const armasHtml = armas.length ? `<div class="jg-bloco" data-bloco-acao="armas"><h4>Ataques de Arma${emFuria ? ` <small class="jg-furia-cab">🔥 Em Fúria: +${furiaBonus} de dano corpo a corpo (Força)</small>` : ''}${descuidadoAtivo ? ` <small class="jg-desc-cab">💥 Descuidado: vantagem no corpo a corpo com Força — mas inimigos têm vantagem contra você até seu próximo turno</small>` : ''}</h4>${armas.map(a => {
      ...
      // Combate 3: só arma EQUIPADA (ou de arremesso, que se saca ao lançar)
      // ataca direto; a não-equipada mostra "🤚 Empunhar", que em combate gasta
      // a Ação do turno (regra 5e de trocar a arma em punho).
      const podeAtacarDireto = equipada || arremesso;
      const btnAcaoArma = podeAtacarDireto
        ...
        : `<button class="btn-mini jg-empunhar" data-empunhar="${esc(a.nome)}" title="Sacar para a mão principal — em combate, gasta a sua Ação deste turno">🤚 Empunhar</button>`;
    ...
    // ----- F5c: Canalizar Divindade do juramento (Paladino) / domínio (Clérigo) -----
    let canalizarSubHtml = '';
    {
      const cSub = classesFicha().find(x => (x.classe === 'Clérigo' && x.nivel >= 2) || (x.classe === 'Paladino' && x.nivel >= 3));
      const ops = (cSub && typeof canalizarSubclasse5e === 'function') ? canalizarSubclasse5e(cSub.subclasse) : [];
      if (cSub && ops.length) {
        const maxC = cSub.classe === 'Clérigo' ? (cSub.nivel >= 18 ? 3 : cSub.nivel >= 6 ? 2 : 1) : 1;
        const restamC = maxC - (f.recursosUsados['Canalizar Divindade'] || 0);
        const icone = cSub.classe === 'Paladino' ? '⚜️' : '🕮';
        canalizarSubHtml = `<div class="jg-bloco jg-expulsar" data-bloco-acao="canalizarsub"><h4>${icone} Canalizar Divindade — ${esc(cSub.subclasse)} <small>(${restamC}/${maxC})</small></h4>` +
          ops.map(o => `<div class="pv-linha"><b>${esc(o.nome)}</b> — ${esc(o.desc)} <button class="btn-mini" data-canalsub="${esc(o.nome)}" data-canaldesc="${esc(o.desc)}" ${restamC > 0 ? '' : 'disabled title="Sem usos — descanse"'}>✨ Usar</button></div>`).join('') +
          `${restamC > 0 ? '' : '<div class="criador-hint">Sem Canalizar Divindade — recupere num descanso curto.</div>'}` +
          `</div>`;
      }
    }
    ...
    if (window.COMBATE_ATUAL && window.COMBATE_ATUAL.combatentes && window.COMBATE_ATUAL.combatentes.length > 0) {
      ...
      alvoHtml = `
        <div class="jg-modo" style="margin-top: 6px;">
          ...
          ${(alvoSel && (alvoSel.hpAtual || 0) > 0) ? `<button id="jgFurtarBtn" class="btn-mini" title="Tentar furtar um item de ${esc(alvoSel.nome)} (Prestidigitação contra uma CD calculada)">🤏 Furtar</button>` : ''}
        </div>
        ${(alvoSel && jgFurtoAlvo === alvoSel.id && (alvoSel.hpAtual || 0) > 0) ? (() => {
          const itensAlvo = (alvoSel.itens || []).filter(x => typeof x === 'string');
          const opsItens = [...new Set(itensAlvo)].map(n => `<option value="${esc(n)}">${esc(n)}</option>`).join('');
          return `<div class="jg-furto jg-bloco" style="margin-top:6px">
            <div class="pv-linha">🤏 <b>Furtar de ${esc(alvoSel.nome)}</b> <small>(Prestidigitação vs CD calculada — o Mestre concede o item)</small></div>
            <div class="dado-row">
              ${itensAlvo.length
                ? `<select id="jgFurtoItem">${opsItens}</select>`
                : `<input type="text" id="jgFurtoItem" placeholder="qual item? (ou deixe genérico)" maxlength="60" style="width:180px">`}
              <label class="check-chip"><input type="checkbox" id="jgFurtoEquip"> equipado</label>
              <label class="check-chip"><input type="checkbox" id="jgFurtoSint"> sintonizado</label>
              <label class="check-chip"><input type="checkbox" id="jgFurtoMissao"> de missão</label>
              <button id="jgFurtoRolar" class="btn-mini btn-primary">🎲 Tentar furto</button>
            </div>
          </div>`;
        })() : ''}
      `;
    ...
    if (et.emCombate && typeof opcoesTurno === 'function') {
      ...
      turnoHtml = `<div class="jg-bloco jg-turno">
        ...
        ${(() => { const reac = (typeof reacoesDoPC === 'function') ? reacoesDoPC(f) : []; return reac.length ? `<div class="jg-turno-dicas jg-reacoes"><h5>⚡ Suas reações <small>(fora do seu turno, 1×/rodada)</small></h5>${reac.map(x => `<div class="pv-linha">${esc(x)}</div>`).join('')}</div>` : ''; })()}
    ...
    // 20.5: seções de referência colapsáveis (accordion) — desafogam a rolagem
    // no celular. `aberto=false` recolhe por padrão (o conteúdo continua a 1 toque).
    const colapso = (titulo, html, aberto, solo) => html
      ? `<details class="jg-colapso${solo ? ' solo' : ''}"${aberto ? ' open' : ''}><summary>${titulo}</summary>${html}</details>` : '';
    ...
    $('modalJogoBody').innerHTML = `
      <div class="jg-header">
        ...
        <div class="jg-rest">
          ...
          ${window.EH_MESTRE
            ...
            : '<span class="criador-hint-inline" title="Slots de magia e recursos só voltam quando o Mestre concede o descanso">😴 O descanso é ativado pelo Mestre</span>'}
      ...
      ${(() => {
        ...
        if (!t.euNaVez) return `<div class="jg-vez jg-vez-aguarda"><span>⏳ Em combate (Rodada ${t.rodada}) · Vez de <b>${esc(t.atualNome)}</b> — fora do seu turno só dá para usar <b>Reação</b>${ficha.reacaoUsadaRodada === t.rodada ? ' (já usada nesta rodada)' : ''}</span></div>`;
        // T3: marcadores de economia de ação (só na minha vez), atrelados à chave do turno
        const g = acoesDoTurno(t.chave);
        const maxAtq = (typeof totalAtaques === 'function') ? totalAtaques(f) : 1;
        const chips = ACOES_TURNO.map(a => {
          const gasto = g[a.chave];
          const extra = (a.chave === 'acao' && gasto && g.acaoTipo === 'ataque' && maxAtq > 1) ? ` <small>${g.ataques || 1}/${maxAtq} golpes</small>` : '';
          return `<button class="jg-acao-chip ${gasto ? 'gasta' : ''}" data-acaoturno="${a.chave}" title="${gasto ? 'Usado — clique para desmarcar' : 'Disponível — clique ao usar'}">${a.icone} ${a.rotulo} ${gasto ? '✔' : ''}${extra}</button>`;
        }).join('')
          + `<button class="jg-acao-chip jg-acao-reset" data-reset-economia="1" title="Reinicia a economia do turno (Surto de Ação, Pressa, correção do Mestre)">↩️ Novo turno</button>`;
        ...
        if (canalizarSubHtml) cats.push(['canalizarsub', '⚜️', 'Canalizar Divindade (subclasse)']);
        ...
        return `<div class="jg-vez jg-vez-eu"><span>⚔️ <b>É a sua vez!</b> · Rodada ${t.rodada}</span>
             <button id="jgFinalizarTurno" class="btn-primary">🔚 Encerrar meu turno</button></div>
      ...
      <div class="jg-cols">
        ...
        <div>${armasHtml}${punicaoHtml}${furtivoHtml}${expulsarHtml}${canalizarSubHtml}${inspiracaoHtml}${feiticariaHtml}${kiHtml}${guerreiroHtml}${magoHtml}${impMaosHtml}${acaoLadinaHtml}${conscienciaHtml}${castHtml}${aurasHtml}${concHtml}${avisosHtml}${colapso('🎒 Bolsa & Equipamento', inventarioHtml, true, true)}${sintHtml}${colapso('📖 Magias (preparar/grimório)', magiasHtml, false, true)}${colapso('📜 Características de Classe', caracHtml, false, false)}${colapso('📖 História & Personalidade', historiaHtml, false, true)}</div>
      </div>
    `;
  ...
  function wire() {
    ...
    if ($('jgDescCurto')) $('jgDescCurto').onclick = () => { descansoCurto(); log('Descanso curto'); };
    if ($('jgDescLongo')) $('jgDescLongo').onclick = () => { descansoLongo(); log('Descanso longo — recuperado'); };
    ...
    document.querySelectorAll('[data-atacararma]').forEach(b => b.onclick = async () => {
      ...
      // Combate 2b: economia — atacar gasta a Ação (ou a Bônus, se for a mão
      // secundária); o Ataque Extra permite mais golpes dentro da mesma Ação.
      const custoAtq = /\(b[ôo]nus\)/i.test(armaNome || '') ? 'bonus' : 'acao';
      const bloqAtq = tentarGastarEconomia(custoAtq, 'ataque');
      if (bloqAtq) { log('🚫 ' + bloqAtq); render(); return; }
    ...
    // Combate 3: empunhar outra arma da bolsa (gasta a Ação em combate)
    document.querySelectorAll('[data-empunhar]').forEach(b => b.onclick = () => {
      const nome = b.dataset.empunhar;
      const emCombate = !!chaveVezAtual();
      const bloq = tentarGastarEconomia('acao', 'empunhar');
      if (bloq) { log('🚫 ' + bloq); render(); return; }
      ficha.equipado = ficha.equipado || {};
      const it = (typeof itemCatalogo === 'function') ? itemCatalogo(nome) : null;
      ficha.equipado.maoPrincipal = nome;
      if (it && it.maos === 2) ficha.equipado.maoSecundaria = ''; // arma de duas mãos ocupa as duas
      log(`🤚 Empunhou ${nome}${emCombate ? ' (gastou a Ação deste turno)' : ''}.`);
      salvar();
    });
    ...
    document.querySelectorAll('[data-atributo]').forEach(b => b.onclick = () => rolarTeste('Teste de ' + b.dataset.anome, +b.dataset.bonus));
    // teste de morte
    ...
    if (document.querySelector('[data-reset-economia]')) document.querySelector('[data-reset-economia]').onclick = resetEconomiaTurno;
    ...
    document.querySelectorAll('[data-conjurar]').forEach(b => b.onclick = () => {
      const nome = b.dataset.conjurar;
      // Combate 2b: conjurar gasta a Ação/Bônus/Reação conforme o tempo da magia
      const custoM = custoMagiaTempo(nome);
      const bloqM = tentarGastarEconomia(custoM, 'magia');
      if (bloqM) { log('🚫 ' + bloqM); render(); return; }
      conjurarMagia(nome);
    });
    ...
    // ----- ✨ Expulsar Mortos-Vivos (F2) — Ação -----
    if ($('jgExpulsar')) $('jgExpulsar').onclick = comEconomia('acao', expulsarMortosVivos);
    // F5c: opções de Canalizar Divindade do juramento/domínio (ungated: o custo
    // de ação varia por opção — a descrição diz "ação"/"ação bônus" ao jogador).
    document.querySelectorAll('[data-canalsub]').forEach(b => b.onclick = () => usarCanalizarSubclasse(b.dataset.canalsub, b.dataset.canaldesc));

    // ----- 🎵 Inspiração Bárdica (F2) — Ação Bônus -----
    if ($('jgInspDar')) $('jgInspDar').onclick = comEconomia('bonus', () => darInspiracao(($('jgInspAlvo') || {}).value));
    document.querySelectorAll('[data-inspusada]').forEach(b => b.onclick = () => inspiracaoUsada(+b.dataset.inspusada));

    // ----- 👊 Opções de Ki (F2) — Rajada/Defesa/Vento = Ação Bônus; Golpe
    // Atordoante é um "rider" num acerto (não gasta ação/bônus, só Ki) -----
    const KI_ROTULO = { rajada: 'Rajada de Golpes', defesa: 'Defesa Paciente', vento: 'Passo do Vento', atordoar: 'Golpe Atordoante' };
    document.querySelectorAll('[data-ki]').forEach(b => {
      const k = b.dataset.ki;
      const usar = () => gastarKi(1, KI_ROTULO[k] || 'Ki');
      b.onclick = (k === 'atordoar') ? usar : comEconomia('bonus', usar);
    });

    // ----- 💨 Retomar o Fôlego (Ação Bônus) / ⚡ Surto de Ação (concede Ação extra) -----
    if ($('jgFolego')) $('jgFolego').onclick = comEconomia('bonus', retomarFolego);
    if ($('jgSurto')) $('jgSurto').onclick = () => { surtoDeAcao(); liberarAcaoExtra(); };

    // ----- 🔮 Recuperação Arcana do Mago (F2) -----
    if ($('jgRecArcana')) $('jgRecArcana').onclick = recuperacaoArcana;

    // ----- 🙏 Imposição das Mãos do Paladino (F2) — Ação -----
    if ($('jgImpMaosCurar')) $('jgImpMaosCurar').onclick = comEconomia('acao', () => imposicaoDasMaos(Number(($('jgImpMaosVal') || {}).value) || 1));
    if ($('jgImpMaosDoenca')) $('jgImpMaosDoenca').onclick = comEconomia('acao', imposicaoDoencaVeneno);

    // ----- 🐾 Consciência Primitiva do Patrulheiro (F2) — Ação -----
    if ($('jgConsciencia')) $('jgConsciencia').onclick = comEconomia('acao', conscienciaPrimitiva);

    // ----- ✴️ Fontes de Feitiçaria (F2) — Ação Bônus (converter espaço/ponto) -----
    document.querySelectorAll('[data-esp2pt]').forEach(b => b.onclick = comEconomia('bonus', () => espacoParaPontos(+b.dataset.esp2pt)));
    document.querySelectorAll('[data-pt2esp]').forEach(b => b.onclick = comEconomia('bonus', () => pontosParaEspaco(+b.dataset.pt2esp)));
    ...
    if (fsBtn) fsBtn.onclick = comEconomia('acao', () => transformarEm(($('jgFsForma') || {}).value, 1)); // Forma Selvagem = Ação
    const fsBtnElem = $('jgFsTransformarElem');
    if (fsBtnElem) fsBtnElem.onclick = comEconomia('acao', () => transformarEm(($('jgFsElemental') || {}).value, 2));
    ...
    // ----- 🤏 C2: furto com CD variável (assistente; o Mestre concede o item) -----
    if ($('jgFurtarBtn')) $('jgFurtarBtn').onclick = () => { jgFurtoAlvo = (jgFurtoAlvo === jgAlvoId) ? '' : jgAlvoId; render(); };
    if ($('jgFurtoRolar')) $('jgFurtoRolar').onclick = () => {
      const cb = window.COMBATE_ATUAL;
      const alvoSel = (cb && cb.combatentes) ? cb.combatentes.find(c => c.id === jgAlvoId) : null;
      if (!alvoSel) { log('Selecione um alvo para furtar.'); return; }
      const itemNome = (($('jgFurtoItem') || {}).value || '').trim() || 'um item';
      const itemData = (typeof itemCatalogo === 'function') ? (itemCatalogo(itemNome) || {}) : {};
      let percep = alvoSel.percepcaoPassiva;
      if (!percep && alvoSel.tipo === 'pc' && alvoSel.fichaId && typeof fichas !== 'undefined') {
        const alvoF = fichas.find(f => f.id === alvoSel.fichaId);
        if (alvoF && typeof percepcaoPassiva === 'function') percep = percepcaoPassiva(alvoF);
      }
      const opts = {
        equipado: !!($('jgFurtoEquip') && $('jgFurtoEquip').checked),
        sintonizado: !!($('jgFurtoSint') && $('jgFurtoSint').checked),
        deMissao: !!($('jgFurtoMissao') && $('jgFurtoMissao').checked),
        distraido: !!(cb && cb.ativo), // em combate o alvo está distraído
      };
      const info = cdFurto5e({ peso: itemData.peso, raridade: itemData.raridade, magico: itemData.magico }, { percepcaoPassiva: percep || 12 }, opts);
      const prof = ((ficha.pericias || []).includes('Prestidigitação')) ? pbAtual() : 0;
      const bonus = m(ficha.atributos.des) + prof;
      const r = d20Modo();
      const total = r.v + bonus;
      const res = resultadoFurto5e(total, info.cd);
      const explic = info.fatores.map(fa => `${fa.valor >= 0 ? '+' : ''}${fa.valor} ${fa.motivo}`).join(', ');
      const emoji = res.grau === 'sucesso' ? '✅' : (res.grau === 'quase' ? '😬' : '🚨');
      const msg = `🤏 ${ficha.nome} tenta furtar ${itemNome} de ${alvoSel.nome}: Prestidigitação ${total} (${r.txt}${bonus >= 0 ? '+' : ''}${bonus}) vs CD ${info.cd} [${explic}] → ${emoji} ${res.msg}`;
      log(msg);
      fetch('/api/combate/acao', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ acao: 'defesa_log', msg }) }).catch(() => { });
      render();
    };
