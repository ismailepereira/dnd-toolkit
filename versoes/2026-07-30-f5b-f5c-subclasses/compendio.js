...
const MAGIAS_SUBCLASSE = {
  ...
  // ---------- Bruxo (F5b): magias EXPANDIDAS do patrono, por nível do Bruxo
  // (1,3,5,7,9 → 1º ao 5º círculo). Ficam sempre disponíveis para lançar com o
  // espaço de Pacto (fora do limite de conhecidas). Só magias que existem no
  // compêndio (o teste de integridade garante). ----------
  'O Corruptor (Fiend)': {
    1: ['Mãos Flamejantes', 'Comando'],
    3: ['Raio Ardente (Scorching Ray)'],
    5: ['Bola de Fogo'],
    7: ['Muralha de Fogo (Wall of Fire)'],
    9: ['Golpe Flamejante (Flame Strike)'],
  },
  'O Arquifada (Archfey)': {
    1: ['Dardo Flamejante (Faerie Fire)', 'Enfeitiçar Pessoa (Charm Person)'],
    3: ['Sugestão (Suggestion)'],
    5: ['Lampejar (Blink)', 'Crescimento Vegetal (Plant Growth)'],
    7: ['Polimorfia (Polymorph)', 'Porta Dimensional (Dimension Door)'],
    9: ['Dominar Pessoa'],
  },
  'O Grande Antigo (Great Old One)': {
    1: ['Perdição (Bane)'],
    3: ['Imobilizar Pessoa (Hold Person)', 'Sugestão (Suggestion)'],
    5: ['Dissipar Magia (Dispel Magic)'],
    7: ['Confusão (Confusion)'],
    9: ['Imobilizar Monstro (Hold Monster)'],
  },
};
...
// F5c: opções de Canalizar Divindade ESPECÍFICAS do juramento (Paladino nv3) e
// do domínio (Clérigo nv2). Cada uma gasta 1 uso de Canalizar Divindade — o
// mesmo recurso do Expulsar Mortos-Vivos. São descritivas (o botão gasta o uso
// e loga o efeito); o Mestre resolve salvas/dano com as ferramentas do combate.
const CANALIZAR_SUBCLASSE = {
  // ---------- Juramentos do Paladino (nv3) ----------
  'Juramento da Devoção': [
    { nome: 'Arma Sagrada', desc: 'Como ação, imbui uma arma: +modificador de Carisma nas jogadas de ataque com ela (mín. +1) e ela emite luz plena (6m) por 1 min.' },
    { nome: 'Expulsar os Profanos', desc: 'Como ação, ínferos e mortos-vivos a 9m que possam ver/ouvir você fazem salva de Sabedoria; quem falha fica Expulso por 1 min (foge de você).' },
  ],
  'Juramento dos Anciões': [
    { nome: 'Ira da Natureza', desc: 'Como ação, vinhas espectrais prendem inimigos a 3m: salva de Força OU Destreza ou ficam Impedidos até você usar de novo ou eles se libertarem (repete a salva).' },
    { nome: 'Expulsar os Infiéis', desc: 'Como ação, fadas e ínferos a 9m que possam ver/ouvir você fazem salva de Sabedoria; quem falha fica Expulso por 1 min.' },
  ],
  'Juramento da Vingança': [
    { nome: 'Abjurar Inimigo', desc: 'Como ação, um alvo a 18m faz salva de Sabedoria (ínfero/morto-vivo com desvantagem): na falha fica Amedrontado e com deslocamento 0 por 1 min; no sucesso, deslocamento pela metade.' },
    { nome: 'Voto de Inimizade', desc: 'Como ação bônus, escolha um alvo a 3m: você tem vantagem nas jogadas de ataque contra ele por 1 min (ou até ele cair).' },
  ],
  // ---------- Domínios do Clérigo (nv2) ----------
  'Domínio da Vida': [
    { nome: 'Preservar a Vida', desc: 'Como ação, cure um total de 5×seu nível de PV, distribuídos entre criaturas a 9m — cada uma até metade do PV máximo dela. Não afeta mortos-vivos/constructos.' },
  ],
  'Domínio da Luz': [
    { nome: 'Fulgor da Aurora', desc: 'Como ação, dissipa escuridão mágica a 9m; cada criatura hostil a 9m faz salva de Constituição e sofre 2d10 + seu nível radiante (metade no sucesso).' },
  ],
  'Domínio da Guerra': [
    { nome: 'Investida Guiada', desc: 'Quando faz um ataque, some +10 na jogada de ataque (pode decidir depois de ver o d20, antes de saber o resultado).' },
  ],
  'Domínio do Engano': [
    { nome: 'Invocar a Duplicidade', desc: 'Como ação, cria uma duplicata ilusória sua a 9m (concentração até 1 min): pode conjurar/atacar como se estivesse na posição dela e ganha vantagem em ataques com um aliado perto dela.' },
  ],
  'Domínio do Conhecimento': [
    { nome: 'Conhecimento das Eras', desc: 'Como ação, ganha proficiência em uma perícia ou ferramenta à escolha por 10 minutos.' },
  ],
  'Domínio da Natureza': [
    { nome: 'Encantar Animais e Plantas', desc: 'Como ação, feras e plantas a 9m que possam ver/ouvir você fazem salva de Sabedoria; quem falha fica Enfeitiçado por você por 1 min.' },
  ],
  'Domínio da Tempestade': [
    { nome: 'Fúria Destrutiva', desc: 'Ao rolar dano de trovão ou elétrico, causa o dano MÁXIMO em vez de rolar.' },
  ],
};
function canalizarSubclasse5e(subclasse) { return CANALIZAR_SUBCLASSE[subclasse] || []; }
...
function rotuloMagiaSubclasse(subclasse) {
  ...
  if (/(Corruptor|Arquifada|Grande Antigo)/.test(subclasse || '')) return 'patrono';
  return 'subclasse';
}
