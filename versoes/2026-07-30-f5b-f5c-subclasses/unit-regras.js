...
t('F5b: patronos do Bruxo têm magias expandidas e rótulo "patrono"', () => {
  ['O Corruptor (Fiend)', 'O Arquifada (Archfey)', 'O Grande Antigo (Great Old One)'].forEach(p => {
    assert.ok(ctx.MAGIAS_SUBCLASSE[p], `patrono sem tabela: ${p}`);
    assert.strictEqual(ctx.rotuloMagiaSubclasse(p), 'patrono');
    // no N5 o Bruxo já tem as de círculo 1-3 acumuladas
    assert.ok(ctx.magiasSubclasse5e(p, 5).length >= 3, `poucas magias no N5 p/ ${p}`);
  });
});

t('F5c: Canalizar de subclasse — chaves válidas e opções bem formadas', () => {
  const todas = [];
  Object.keys(ctx.SUBCLASSES).forEach(cl => ctx.SUBCLASSES[cl].opcoes.forEach(o => todas.push(o.nome)));
  Object.keys(ctx.CANALIZAR_SUBCLASSE).forEach(sc => {
    assert.ok(todas.includes(sc), `subclasse desconhecida na tabela de Canalizar: ${sc}`);
    ctx.CANALIZAR_SUBCLASSE[sc].forEach(o => assert.ok(o.nome && o.desc, `opção incompleta em ${sc}`));
  });
  assert.ok(ctx.canalizarSubclasse5e('Juramento da Vingança').some(o => /Voto de Inimizade/.test(o.nome)));
  assert.strictEqual(ctx.canalizarSubclasse5e('Inexistente').length, 0);
});
...
t('descanso curto: recupera recursos "curto" e pacto; não zera slots nem PV', () => {
  const f = {
    classe: 'Clérigo', nivel: 5, hpMax: 30, hpAtual: 10, atributos: { car: 10, sab: 16 },
    slotsUsados: { 1: 2, 2: 1 }, pactoUsados: 1,
    recursosUsados: { 'Canalizar Divindade': 1 }, // Clérigo: 'curto'
  };
  ctx.aplicarDescansoCurto5e(f);
  assert.strictEqual(f.recursosUsados['Canalizar Divindade'], 0); // recuperado
  assert.strictEqual(f.pactoUsados, 0);                            // pacto volta no curto
  assert.strictEqual(f.hpAtual, 10);                               // curto NÃO cura PV
  assert.strictEqual(f.slotsUsados[1], 2);                         // curto NÃO devolve slots
});

t('economia 2b: 1ª Ação livre; 2ª Ação (magia após atacar) bloqueia', () => {
  const r1 = ctx.economiaAcao5e(null, 'acao', 'ataque', 1);
  assert.strictEqual(r1.bloqueio, null);
  assert.strictEqual(r1.gasto.acao, true);
  assert.strictEqual(r1.gasto.acaoTipo, 'ataque');
  // já atacou (Ação usada) → conjurar magia de ação bloqueia
  const r2 = ctx.economiaAcao5e(r1.gasto, 'acao', 'magia', 1);
  assert.ok(/Ação atacando/.test(r2.bloqueio));
});

t('economia 2b: Ataque Extra permite 2 golpes na mesma Ação, o 3º bloqueia', () => {
  let g = ctx.economiaAcao5e(null, 'acao', 'ataque', 2).gasto;       // golpe 1
  const r2 = ctx.economiaAcao5e(g, 'acao', 'ataque', 2);            // golpe 2
  assert.strictEqual(r2.bloqueio, null);
  assert.strictEqual(r2.gasto.ataques, 2);
  const r3 = ctx.economiaAcao5e(r2.gasto, 'acao', 'ataque', 2);     // golpe 3 → bloqueia
  assert.ok(/todos os ataques/.test(r3.bloqueio));
});

t('rolarDadosGrupo (20.6): soma dados + modificador; d20 detecta natural', () => {
  // rng fixo em 0.5 → cada dX = 1 + floor(0.5*X)
  const meio = () => 0.5;
  const r = ctx.rolarDadosGrupo(3, 6, 2, 'normal', meio); // 3d6: cada = 1+floor(3)=4 → 12 +2 = 14
  assert.strictEqual(r.dados.join(','), '4,4,4');
  assert.strictEqual(r.total, 14);
  const rc = ctx.rolarDadosGrupo(1, 20, 0, 'normal', () => 0.999); // d20 = 20 → crítico
  assert.strictEqual(rc.dados[0], 20);
  assert.strictEqual(rc.natural, 'critico');
});

t('rolarDadosGrupo (20.6): vantagem/desvantagem pega maior/menor de 2d20', () => {
  const seq = [0.0, 0.9]; let i = 0; const rng = () => seq[i++ % seq.length]; // 1, depois 19
  const v = ctx.rolarDadosGrupo(1, 20, 3, 'vantagem', rng);
  assert.strictEqual(v.escolhido, 19); // maior de [1,19]
  assert.strictEqual(v.total, 22);     // 19 + 3
  i = 0;
  const d = ctx.rolarDadosGrupo(1, 20, 0, 'desvantagem', rng);
  assert.strictEqual(d.escolhido, 1);  // menor de [1,19]
});

t('reacoesDoPC: Mago com Escudo e Ladino nv5 detectam suas reações', () => {
  const mago = ctx.reacoesDoPC({ classe: 'Mago', nivel: 5, magias1: ['Escudo', 'Mísseis Mágicos'] });
  assert.ok(mago.some(r => /Escudo/.test(r)), mago.join(','));
  const ladino = ctx.reacoesDoPC({ classe: 'Ladino', nivel: 5, magias1: [] });
  assert.ok(ladino.some(r => /Esquiva Sobrenatural/.test(r)), ladino.join(','));
  const guerreiro = ctx.reacoesDoPC({ classe: 'Guerreiro', nivel: 3, magias1: [] });
  assert.strictEqual(guerreiro.length, 0); // sem reação defensiva conhecida
});

t('Teste de Morte: 3 falhas mata; 1 natural conta 2; 20 recupera 1 PV', () => {
  const f = {};
  ctx.testeMorte5e(f, 5);  // falha (1)
  ctx.testeMorte5e(f, 8);  // falha (2)
  const r3 = ctx.testeMorte5e(f, 7); // falha (3) → morto
  assert.strictEqual(f.morteFalhas, 3);
  assert.ok(r3.morto && f.status === 'morto');

  const g = {};
  const r1 = ctx.testeMorte5e(g, 1); // 1 natural = 2 falhas
  assert.strictEqual(g.morteFalhas, 2);
  assert.ok(!r1.morto);

  const h = { hpAtual: 0 };
  const rn = ctx.testeMorte5e(h, 20); // 20 natural = de pé com 1 PV
  assert.strictEqual(h.hpAtual, 1);
  assert.ok(rn.recuperou && h.morteSucessos === 0 && h.morteFalhas === 0);
});

t('Teste de Morte: 3 sucessos estabiliza (inconsciente, não morto)', () => {
  const f = {};
  ctx.testeMorte5e(f, 12); ctx.testeMorte5e(f, 15);
  const r = ctx.testeMorte5e(f, 10);
  assert.ok(r.estavel && f.morteSucessos === 3 && f.status !== 'morto');
});

t('C2 furto: CD sobe com equipado + raro, com fatores explicando', () => {
  const r = ctx.cdFurto5e({ raridade: 'Raro', peso: 0.5 }, { percepcaoPassiva: 13 }, { equipado: true });
  // base 13 + raro 3 + equipado 5 = 21
  assert.strictEqual(r.cd, 21, 'CD esperada 21, veio ' + r.cd);
  assert.ok(r.fatores.some(f => /equipado/.test(f.motivo) && f.valor === 5));
  assert.ok(r.fatores.some(f => /raro/.test(f.motivo) && f.valor === 3));
});

t('C2 furto: distração e ajuda reduzem a CD (piso 5)', () => {
  const r = ctx.cdFurto5e({}, { percepcaoPassiva: 8 }, { distraido: true, ajuda: true });
  assert.strictEqual(r.cd, 5); // 8 -3 -2 = 3 → piso 5
});

t('C2 furto: grau — sucesso, quase (≤3) e flagrado', () => {
  assert.strictEqual(ctx.resultadoFurto5e(15, 15).grau, 'sucesso');
  assert.strictEqual(ctx.resultadoFurto5e(13, 15).grau, 'quase');   // faltou 2
  assert.strictEqual(ctx.resultadoFurto5e(9, 15).grau, 'flagrado'); // faltou 6
});

t('economia 3: empunhar outra arma gasta a Ação (depois não ataca no mesmo turno)', () => {
  const r1 = ctx.economiaAcao5e(null, 'acao', 'empunhar', 2);
  assert.strictEqual(r1.bloqueio, null);
  assert.strictEqual(r1.gasto.acaoTipo, 'empunhar');
  const r2 = ctx.economiaAcao5e(r1.gasto, 'acao', 'ataque', 2); // sacou a arma → sem ataque neste turno
  assert.ok(/trocando de arma/.test(r2.bloqueio), r2.bloqueio);
});

t('economia 2b: Ação, Bônus e Reação são independentes', () => {
  let g = ctx.economiaAcao5e(null, 'acao', 'magia', 1).gasto;      // gasta Ação (magia)
  const rb = ctx.economiaAcao5e(g, 'bonus', 'magia', 1);           // Bônus ainda livre
  assert.strictEqual(rb.bloqueio, null);
  assert.strictEqual(rb.gasto.bonus, true);
  const rb2 = ctx.economiaAcao5e(rb.gasto, 'bonus', 'magia', 1);   // 2ª Bônus bloqueia
  assert.ok(/Ação Bônus/.test(rb2.bloqueio));
  const rr = ctx.economiaAcao5e(rb.gasto, 'reacao', 'magia', 1);   // Reação livre
  assert.strictEqual(rr.bloqueio, null);
});

t('descanso longo: cura PV, zera slots, pacto e recursos', () => {
  const f = {
    classe: 'Mago', nivel: 6, hpMax: 30, hpAtual: 4, pvTemp: 3, atributos: { car: 10 },
    slotsUsados: { 1: 2, 3: 1 }, pactoUsados: 1, recursosUsados: { X: 2 }, dvUsados: 6,
  };
  ctx.aplicarDescansoLongo5e(f);
  assert.strictEqual(f.hpAtual, 30);
  assert.strictEqual(f.pvTemp, 0);
  assert.strictEqual(Object.keys(f.slotsUsados).length, 0);
  assert.strictEqual(f.pactoUsados, 0);
  assert.strictEqual(Object.keys(f.recursosUsados).length, 0);
  assert.strictEqual(f.dvUsados, 3); // recupera metade do nível (6/2=3): 6-3=3
});
